...
please also install Visual Studio ([https://www.visualstudio.com](https://www.visualstudio.com)).  Note that
on Windows machines, after installing Visual Studio, all commands below must be run in powershell, rather
than in the standard Windows command - you can access powershell from the standard Windows command line by
typing: `$ powershell`.  It is also necessary to run the command terminal with Administrator privileges.<br>
 ...
 * In `/var/www/AIFCOMSSwithCUPredictorTest/predict/AIFCOMSS.html`, change the path of `AIFpred.php` to the correct location, <i>if</i> the correct location happens to be different than `http://localhost/AIFCOMSSwithCUPredictorTest/predict/AIFpred.php` in your setup.
