 ...
 * Mac OS: Mac OS X already comes with both Apache and PHP installed (you just need to turn them on, and if you have a pre-High Sierra, i.e. Mac OS 10.13, OS version, then you'll need to upgrade your PHP from v.5 to v.7.1, but you don't need to install either Apache or PHP ab initio).  You will, however, need to install MySQL.  Instructions for doing all this setup in High Sierra can be found (for example) [here](https://websitebeaver.com/set-up-localhost-on-macos-high-sierra-apache-mysql-and-php-7-with-sslhttps); and you can simply google `mac apache` for instructions on setting the AMP stack up in earlier OS X versions.  And, as also mentioned below, ignore any optional steps - the goal is just to get PHP pages to display.
